...
interface EditMonthlyOKRsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  year: number;
  month: number;
...
export function EditMonthlyOKRsDialog({ isOpen, onClose, year, month }: EditMonthlyOKRsDialogProps) {
  const monthlyOKRs = useQuery(api.monthlyReview.getMonthlyOKRs, { year, month });
  const updateMonthlyOKRs = useMutation(api.monthlyReview.updateMonthlyOKRs);
  ...
  useEffect(() => {
    if (monthlyOKRs && isOpen) {
      if (monthlyOKRs.length > 0) {
        setOkrs(monthlyOKRs);
      } else {
        setOkrs([{
          area: "Wealth",
  ...
  const addKeyResult = (okrIndex: number) => {
    setOkrs((prev) => {
      const updated = [...prev];
      updated[okrIndex].keyResults.push({ description: "", target: 1, unit: "" });
      return updated;
    });
  };
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await updateMonthlyOKRs({
        year,
        month,
        okrs: validOKRs
      });
      toast.success(`Monthly OKRs updated!`);
      onClose();
    } catch (error) {
      ...
      <DialogContent className="w-[95vw] max-w-4xl max-h-[85vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="font-orbitron">
            Edit {monthNames[month - 1]} {year} OKRs
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="flex items-start gap-2">
                <div className="flex-1 space-y-3">
                  ...
                  <Input
                    ...
                    placeholder="Objective (e.g., 'Increase revenue')"
                  />
