...
export function MonthlyComparison({ monthlyStats }: MonthlyComparisonProps) {
  ...
  return (
    <Card className="p-6 dark:border-[rgba(0,230,118,0.15)] border-[rgba(76,175,80,0.35)] dark:bg-card/50 bg-white/80
      shadow-sm hover:shadow-xl transition-all duration-300 rounded-2xl
      dark:hover:border-[rgba(0,230,118,0.25)] hover:border-[rgba(76,175,80,0.5)]">
      ...
      <div className="space-y-3">
        {displayStats.map((stat) => (
          <div key={stat.month} className="flex items-center gap-4">
            {/* Month Label */}
            <div
              className="w-12 text-sm font-medium dark:text-[#E0E0E0] text-[#000000]"
            ...
            {/* Progress Bar */}
            <div className="flex-1 h-6 dark:bg-[#2A2A2E] bg-[#d0d7de] rounded overflow-hidden
              dark:border-[rgba(0,230,118,0.1)] border-[rgba(76,175,80,0.3)] border">
              {stat.score > 0 ? (
                <div
                ...
                <div className="h-full dark:bg-[#2A2A2E] bg-[#d0d7de]" />
              )}
            </div>

            {/* Percentage */}
            <div
              className="w-12 text-right text-sm font-medium dark:text-[#E0E0E0] text-[#000000]"
              style={{ fontFamily: '"Courier New", "Monaco", monospace' }}
            >
